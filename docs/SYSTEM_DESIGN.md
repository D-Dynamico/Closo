...
The complete reference: thesis, layer contracts, data model, invariants, test plan and
stage sequence. `CLAUDE.md` is the short onboarding doc for a new session and points
here; this file is the detail behind it.

**Section numbers are load-bearing.** Docstrings throughout `closo/` cite them — a
comment reading "(12.2)" means section 12.2 of this file. Do not renumber without
grepping first.
