# 2026-08-25 — Stages 7 and 8: full pipeline, then the full UI

Two stages in one session. **Stage 7** wired Layers 2 and 3 into `pipeline.run()`, added
cost metrics, and recorded a live run into a committed cache so the demo replays offline.
**Stage 8** built the three remaining screens on top of that log. Starts from `7d1c6f2`
with 347 tests passing; ends at 488.
...
## Substeps 3–6 — the three remaining screens — 2026-08-25

**Done:** Live run, Exception drill-down and Escalation queue, plus the colour-code pass.
27 new tests; full suite **488 passing**. Stage 8's exit criterion (§12.7 end-to-end in
airplane mode) is met, including one test that walks the whole checklist as a sequence.

**Decisions:**

- **All three screens read the audit log, never the in-memory run.** One `RunStory` per
  screen, narrated from `events`. A replayed run therefore renders through identical code —
  and it is the only source that carries a settlement-side exception's investigation at all.

- **The Live-run animation plays once per run**, with a "Play again" button. Re-animating on
  every sidebar click would be theatre rather than information, and it would make the screen
  slow to leave and come back to.

- **`st.status` state is `error` only for a verifier rejection.** E9 and E10 are *supposed*
  to end escalated; marking their blocks as errors would tell a judge the run failed on the
  four records where it did exactly what it was designed for.

- **The drill-down opens on a fee-schedule case.** §13's Stage 9 says an E4 tells the best
  story, so it is what loads rather than something to navigate to under time pressure.

- **The escalation queue lists four items where the Scorecard counts two.** The scorecard
  counts bank credits and an E9 has none — but money that never arrived is exactly what
  belongs in a queue a human works through. Both numbers are right for their own screen, and
  the test says so in its assertion message rather than leaving the next reader to wonder.

- **One `STATE_COLORS` mapping for the chart and the text.** The palette was fixed in §10 and
  used in exactly one place, so the three new screens spoke in words alone and a glance at
  them meant something different from a glance at the Scorecard. A hex written twice agrees
  by luck until one copy changes.

- **Nothing is said in colour alone.** Every tinted label wraps its own words; a test strips
  the markup and fails if a tint ever wraps a bare mark.

**Surprises — three, all from clicking rather than from tests:**

- **Every block header carried two ticks.** `st.status` draws its own for a completed block,
  and the code prefixed another. Worse than duplication: on a screen where ✓ is the
  *verifier's* vocabulary, Streamlit's "this block finished" tick reads as "this
  reconciled" — so all four honest escalations would have shown a ✓ beside them. The mark is
  now left to Streamlit and the outcome is carried by words, which cannot be misread as a
  verdict.

- **"verdict rejected" was the investigator's own wording**, surfacing on the escalation
  screen directly under a line reading "0 proposed and rejected by the verifier". Three
  different things can refuse a verdict here — the investigator (malformed output), the
  verifier (bad math), and the pipeline (double-counted credit) — and the queue shows all
  three, so each now says which one acted.

- **The UI fixture was only isolating its first test.** `get_audit_log` is a
  `@st.cache_resource`, and that cache outlives the `AppTest`, so every test after the first
  was handed the first one's database however carefully `DB_PATH` was patched. Nothing ever
  failed, because each test was self-consistent inside its own run — it surfaced only when a
  test seeded a database and the app read a different one. The fixture now clears both
  Streamlit caches.

- Also worth recording: a test asserting *every* Layer 1 reason has an unblock hint found
  `outside_date_window` had none, and would have printed "needs a human to look at the source
  records" — true of everything on that screen and therefore useless.

**Verified:** 488 passing. Mutation-tested eight ways across the screens and the colour code,
all caught — honest escalations marked as errors, strikethrough dropped from rejected
hypotheses, only failed verifier checks rendered, the unblock hint collapsed to the fallback,
rejections no longer sorted first, the drill-down opening on the wrong exception, a fourth
colour introduced, and a tint wrapping a bare mark instead of words.

---

## Stage 8 complete — 2026-08-25

**Exit (WORKFLOWS §13): "§12.7 checklist passes end-to-end in airplane mode."** Met, as
scripted tests *and* as a browser pass through all five screens. 488 tests passing.

The §12.7 checklist, item by item:

| Check | Where |
|---|---|
| `streamlit run` boots with no network | the app builds a `CachedLLMClient`; the cost line reads `0 API request(s)` |
| Replay-last-run works airplane-mode | `test_the_whole_demo_runs_in_airplane_mode`, and the replay now carries the investigation too |
| All three colours appear on the Scorecard | tier-bar test asserts three colours **and** a non-zero count in each |
| Drill-down renders a full evidence trail for one E4 | opens on it by default: hypothesis, three ruled-out hypotheses, six tool calls with results, arithmetic, five verifier checks |
| Escalation screen shows a verifier-rejected verdict | driven over a crafted run, since the good run contains none |

**On the crafted rejection.** The cached demo run has no verifier rejection — every verdict
either verified or was honestly unresolvable — so the one screen element §12.7 names
specifically had nothing to render. Rather than seed a fake rejection into the demo data,
the test writes one into its own throwaway audit database and drives the real screen over
it. The demo data stays a record of what actually happened.

**Where Stage 8 leaves the project**

Five screens, all live, on 488 offline tests. The product is demoable end to end: run,
watch, read the numbers, drill into the fee-schedule case, work the queue, replay the lot
with the network unplugged.

**Open for Stage 9 and beyond:**

- **Stage 9 — demo polish.** README quickstart and a 4-minute script, two timed rehearsals,
  fresh-clone-to-demo under two minutes (the clone itself was verified in Stage 7). The
  drill-down already opens on the E4, so the one thing to decide is the order of the beats.
- **`app/streamlit_app.py` is now the largest module in the repo.** It splits cleanly into
  `pages/`, which §4 already anticipates.
- **Three modules still over ~300 code lines**: `generator.py` (392),
  `layer2_investigator.py` (341), `pipeline.py` (333).
- **Section-number ambiguity** (`8.1`, `10.1` each resolve two ways) is pinned by tests but
  not fixed.
- **The response cache is a live dependency of the demo.** The zero-misses test is the
  alarm; re-recording costs ~48 requests.
- The first suite run of the session failed both subprocess-based import tests and has been
  green in every run since. Still unexplained.
