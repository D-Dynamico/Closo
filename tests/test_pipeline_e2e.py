...
# --------------------------------------------------------------------------
# A replayed run carries its investigation, not just its statuses
# --------------------------------------------------------------------------
#
# Stage 8's drill-down and escalation screens read verdicts and verifier
# results. Replay is the path the demo falls back to when the network dies,
# so a replayed run that carries only terminal states would render those
# two screens empty in exactly the situation they exist for.


@pytest.fixture
def replayed_pair(batch: GeneratedBatch, log: AuditLog):
    live = run(batch, seed=DEMO_SEED, audit=log, run_id="run_investigated",
               investigator=cached_agent(batch))
    return live, replay("run_investigated", log)


def test_a_replay_restores_every_verdict(replayed_pair) -> None:
    live, replayed = replayed_pair
    assert replayed.verdicts == live.verdicts


def test_a_replay_restores_every_verifier_result(replayed_pair) -> None:
    live, replayed = replayed_pair
    assert replayed.verifications == live.verifications


def test_a_settlement_side_verdict_survives_a_replay(replayed_pair) -> None:
    """The case `resolutions` cannot carry. An E9 exception has no bank
    credit, so it has no row - and its investigation would disappear on
    replay while every visible number stayed correct.

    The exceptions are taken from Layer 1 rather than inferred from the
    verdicts, because inferring them is how this test first managed to
    compare exception ids against bank transaction ids and pass no matter
    what the replay did.
    """
    live, replayed = replayed_pair
    settlement_side = [
        item.exception_id for item in live.layer1.exceptions
        if item.bank_txn_id is None
    ]
    assert settlement_side, "the demo set must still contain E9 exceptions"
    for exception_id in settlement_side:
        assert replayed.verdicts[exception_id] == live.verdicts[exception_id]
        assert exception_id in replayed.verifications


def test_the_evidence_trail_survives_a_replay(replayed_pair) -> None:
    """12.7 wants a full evidence trail on the drill-down. Every tool call
    the investigator made has to come back, not merely the conclusion it
    reached."""
    live, replayed = replayed_pair
    for exception_id, verdict in live.verdicts.items():
        assert replayed.verdicts[exception_id].evidence == verdict.evidence
    assert any(v.evidence for v in replayed.verdicts.values())


def test_the_verifier_checklist_survives_a_replay(replayed_pair) -> None:
    """The drill-down shows per-check ✓/✗. A replay that kept only
    `passed` would reduce the screen that carries the project's whole
    argument to a single boolean."""
    live, replayed = replayed_pair
    checked = [v for v in replayed.verifications.values() if len(v.checks) > 1]
    assert checked, "at least one verdict must come back with its full checklist"
    for exception_id, result in live.verifications.items():
        assert replayed.verifications[exception_id].checks == result.checks


def test_the_fee_schedule_anomaly_survives_a_replay(replayed_pair) -> None:
    """The sentence the E4 drill-down is built around - which schedule was
    applied and which was active. It is generated by the verifier at run
    time and is not recomputable from a status."""
    live, replayed = replayed_pair
    anomalies = {
        exception_id: result.schedule_anomaly
        for exception_id, result in live.verifications.items()
        if result.schedule_anomaly
    }
    assert anomalies
    for exception_id, text in anomalies.items():
        assert replayed.verifications[exception_id].schedule_anomaly == text


def test_replayed_money_is_still_decimal_not_float(replayed_pair) -> None:
    """The verdict goes to JSON and back. A float creeping in on the return
    trip would put a lost paisa on the drill-down's arithmetic block, which
    is the one screen that claims to show exact figures."""
    _, replayed = replayed_pair
    blocks = [
        v.proposed_match.arithmetic for v in replayed.verdicts.values()
        if v.proposed_match is not None
    ]
    assert blocks
    for block in blocks:
        for value in (block.gross, block.mdr, block.gst, block.rounding, block.net):
            assert isinstance(value, Decimal)


def test_a_run_with_no_investigator_replays_to_an_empty_investigation(
    batch: GeneratedBatch, log: AuditLog
) -> None:
    """Layer 1 alone records no verdicts, so a replay of it must produce
    none - rather than failing on an event type that was never written."""
    run(batch, seed=DEMO_SEED, audit=log, run_id="run_layer1_only")
    replayed = replay("run_layer1_only", log)
    assert replayed.verdicts == {} and replayed.verifications == {}
    assert replayed.statuses
